...
public class Mynn_viewWeight_1linear_auto {
	...
	public Mynn_viewWeight_1linear_auto(
			List<View> views,
	...
	public void net4ViewUserListLinkAndForward(Map<CNN4ViewUser,List<NNInterface>> viewUserDocAverageListMap) throws Exception{
		for(Map.Entry<CNN4ViewUser,List<NNInterface>> entry:viewUserDocAverageListMap.entrySet()){
			...
			net4User.average.forward(net4User.view.viewWeight);
	...
	public static void expRun(String sourceBase,String destBase,List<ViewInfo> viewInfos) throws Exception{
		...
		for(int fold=0;fold<totalFold;++fold){
			...
			for(ViewInfo vinfo:viewInfos){
				views.add(new View(vinfo,trainIds,validationIds,testIds));
			}
			...
			if(views.size()>1){
				for(int i=0;i<views.size();++i){
					if(i<views.size()-1)
						destName+=views.get(i).viewName+"-weight"+views.get(i).viewWeight+"+";
					else
						destName+=views.get(i).viewName+"-weight"+views.get(i).viewWeight+"";
				}
			}
			GlobleLog.setLogDir(FileTool.backReplaceDirNode(destBase+"5fold\\"+fold+"\\", "5fold", destName));
			Mynn_viewWeight_1linear_auto main = new Mynn_viewWeight_1linear_auto(
					views,
	...
	public static void main(String[] args) throws Exception{
		...
		for(int i=9;i>2;--i){
			String view4Name="Weibos-10x20";
			double view4Weight=(i*1.0)/10.0f;
			List<ViewInfo> views=new ArrayList<ViewInfo>();
			addExpView(sourceBase,view4Name,views,view4Weight);
			
			String view5Name="Tag-friTagVec";
			addExpView(sourceBase,view5Name,views,1-view4Weight);
			
			expViewInfos.add(views);
		}


		for(List<ViewInfo> viewInfos:expViewInfos){
			expRun(sourceBase,destBase,viewInfos);
		}
	}

	public static void addExpView(String sourceBase,String viewName,List<ViewInfo> views,double viewWeight) throws IOException
	{
		String embeddingFileView =sourceBase+viewName+"\\wordVec\\word_vector.txt";
		String allDataView=sourceBase+viewName+"\\textData\\allReview.txt";
		int embeddingLengthView =WYIO.readVecDiem(embeddingFileView, " ");
		views.add(new ViewInfo(viewName,embeddingFileView,embeddingLengthView,allDataView,viewWeight));
	}
